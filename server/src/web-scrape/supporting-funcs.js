...
export function removeResizing(str) {
    return str.slice(0, str.indexOf('._')) + '.jpg';
}
export async function getData(page) {
    let title, image, price, description;
    try {
    ...
    try {
        image = await page.$eval('#landingImage', el => el.getAttribute('src').trim());
    ...
    try {
        price = parseFloat(await page.$eval('.a-price .a-offscreen', el => el.textContent.trim().slice(1)));
    ...
    try {
        description = await page.$$eval('#feature-bullets li span', el => el.map(e => e.textContent.trim()));
    }
    catch (e) {
        console.error(e);
    }
...
async function clickImage(page, i, url) {
    try {
...
export async function* visitSubLinks(page, baseUrl, subLinks) {
    let count = 0;
...
export async function getSubLinks(page, baseUrl) {
    ...
    return await page.$$eval('a[href*="s?bbn"]', links => links.map(link => link.getAttribute('href')).slice(0, 50));
}
